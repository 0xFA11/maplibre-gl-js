...
export function drawFillExtrusionOutline(painter: Painter, source: SourceCache, layer: FillExtrusionStyleLayer, coords: Array<OverscaledTileID>) {
...
function drawExtrusionTiles(
    ...
    for (const coord of coords) {
        ...
        const bucket: FillExtrusionOutlineBucket = (tile.getBucket(layer) as any);
        if (!bucket) continue;

        const programConfiguration = bucket.programConfigurations.get(layer.id);
        const program = painter.useProgram('fillExtrusionOutline', programConfiguration);
